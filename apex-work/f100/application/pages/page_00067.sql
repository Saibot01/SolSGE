...
,p_item_plug_id=>wwv_flow_imp.id(12007879735524747)
,p_item_source_plug_id=>wwv_flow_imp.id(12776927328370633)
,p_item_default=>'SELECT WKSP_WORKPLACE.FN_OFICINA_USUARIO_V2(V(''APP_USER'')) FROM DUAL'
...
,p_lov=>wwv_flow_string.join(wwv_flow_t_varchar2(
'SELECT descripcion d, codigo_oficina r FROM WKSP_WORKPLACE.OFICINAS',
'WHERE codigo_oficina = WKSP_WORKPLACE.FN_OFICINA_USUARIO_V2(V(''APP_USER''))'))
,p_cHeight=>1
...
,p_display_as=>'NATIVE_SELECT_LIST'
,p_lov=>wwv_flow_string.join(wwv_flow_t_varchar2(
'SELECT DESCRIPCION d, ID_TALONARIO r',
'FROM WKSP_WORKPLACE.V_TALONARIOS_DISPONIBLES',
'WHERE ID_OFICINA = NVL(:P67_ID_OFICINA, WKSP_WORKPLACE.FN_OFICINA_USUARIO_V2(V(''APP_USER'')))',
'  AND TIPO_COMPROBANTE = ''FA''',
'ORDER BY ID_TALONARIO'))
,p_lov_cascade_parent_items=>'P67_ID_OFICINA'
...
'BEGIN',
'  SELECT ID_PERSONA, NVL(TOTAL,0)',
'    INTO :P67_ID_CLIENTE, :P67_TOTAL_MONEDA_LOCAL',
'    FROM WKSP_WORKPLACE.ORDENES_VENTA',
'   WHERE ID_ORDEN = :P67_ID_ORDEN_VENTA;',
'EXCEPTION WHEN NO_DATA_FOUND THEN',
'  :P67_ID_CLIENTE := NULL;',
'  :P67_TOTAL_MONEDA_LOCAL := NULL;',
'END;'))
,p_attribute_02=>'P67_ID_ORDEN_VENTA'
,p_attribute_03=>'P67_ID_CLIENTE,P67_TOTAL_MONEDA_LOCAL'
,p_attribute_04=>'N'
...
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(12792268807100000)
,p_process_sequence=>3
,p_process_point=>'AFTER_SUBMIT'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Validar transicion OV a FACTURADO'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'DECLARE',
'  v_estado_ov WKSP_WORKPLACE.ORDENES_VENTA.ESTADO%TYPE;',
'BEGIN',
'  IF :P67_ID_ORDEN_VENTA IS NULL THEN',
'    apex_error.add_error(',
'      p_message => ''Debes seleccionar un presupuesto APROBADO para facturar.'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'    RETURN;',
'  END IF;',
'  SELECT ESTADO INTO v_estado_ov',
'    FROM WKSP_WORKPLACE.ORDENES_VENTA WHERE ID_ORDEN = :P67_ID_ORDEN_VENTA;',
'  IF WKSP_WORKPLACE.FN_PUEDE_TRANSICION_OV(v_estado_ov, ''FACTURADO'') <> ''S'' THEN',
'    apex_error.add_error(',
'      p_message => ''El presupuesto #'' || :P67_ID_ORDEN_VENTA || '' no se puede facturar (estado actual: '' || v_estado_ov || '').'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'  END IF;',
'END;'))
...
,p_internal_uid=>12792268807100000
);
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(12792268808100000)
,p_process_sequence=>5
,p_process_point=>'AFTER_SUBMIT'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Validar monto pagado contado'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'DECLARE',
'  v_monto_pago NUMBER;',
'  v_total      NUMBER;',
'BEGIN',
'  IF NVL(TO_CHAR(:P67_FORMA_PAGO), ''0'') <> ''21'' THEN',
'    RETURN; -- solo aplica a contado',
'  END IF;',
'  v_monto_pago := TO_NUMBER(REPLACE(NVL(:P67_MONTO_PAGO,''0''), ''.'', ''''));',
'  v_total      := TO_NUMBER(REPLACE(NVL(:P67_TOTAL_MONEDA_LOCAL,''0''), ''.'', ''''));',
'  IF v_monto_pago < v_total THEN',
'    apex_error.add_error(',
'      p_message => ''El monto recibido ('' || v_monto_pago || '') es menor al total a cobrar ('' || v_total || '').'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'  END IF;',
'END;'))
,p_process_clob_language=>'PLSQL'
...
,p_process_when=>'CREATE'
,p_process_when_type=>'REQUEST_IN_CONDITION'
,p_internal_uid=>12792268808100000
);
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(12792268809100000)
,p_process_sequence=>8
,p_process_point=>'AFTER_SUBMIT'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Reservar nro comprobante'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'BEGIN',
'  IF :P67_ID_TALONARIO IS NULL THEN',
'    apex_error.add_error(',
'      p_message => ''No hay talonario seleccionado. Verifica que tu oficina tenga un talonario FA vigente.'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'    RETURN;',
'  END IF;',
'  -- Leer el tipo del talonario (P67_TIPO_COMPROBANTE es hidden)',
'  SELECT TIPO_COMPROBANTE INTO :P67_TIPO_COMPROBANTE',
'    FROM WKSP_WORKPLACE.TALONARIOS WHERE ID_TALONARIO = :P67_ID_TALONARIO;',
'  -- Reservar atomicamente el numero',
'  :P67_NRO_COMPROBANTE := WKSP_WORKPLACE.FN_OBTENER_COMPROBANTE(:P67_ID_TALONARIO);',
'END;'))
,p_process_clob_language=>'PLSQL'
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
,p_process_when=>'CREATE'
...
,p_process_point=>'AFTER_SUBMIT'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Movimiento de caja contado'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'DECLARE',
'  v_id_caja        NUMBER;',
...
'  v_total          NUMBER;',
'BEGIN',
'  -- Solo cuando es contado',
'  IF NVL(TO_CHAR(:P67_FORMA_PAGO), ''0'') <> ''21'' THEN',
'    RETURN;',
'  END IF;',
'  v_id_caja := WKSP_WORKPLACE.FN_CAJA_ABIERTA_USUARIO(V(''APP_USER''));',
'  IF v_id_caja IS NULL THEN',
'    RAISE_APPLICATION_ERROR(-20040, ''No hay caja abierta del usuario para registrar el ingreso.'');',
'  END IF;',
'  v_total := TO_NUMBER(REPLACE(NVL(:P67_TOTAL_MONEDA_LOCAL, ''0''), ''.'', ''''));',
'',
'  INSERT INTO WKSP_WORKPLACE.MOVIMIENTOS_CAJA (',
'    ID_CLIENTE, ID_CAJA, FECHA, TOTAL_MONEDA_LOCAL,',
...
'    ID_MOVIMIENTO, ID_FORMA_PAGO, MONTO_LOCAL,',
'    MONTO_ORIGEN, MONEDA, TIPO_CAMBIO',
'  ) VALUES (',
'    v_id_movimiento, 21, v_total,',
'    NULL, NVL(:P67_MONEDA, ''PYG''), NULL',
'  );',
'END;'))
,p_process_clob_language=>'PLSQL'
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
,p_process_when=>'CREATE'
,p_process_when_type=>'REQUEST_IN_CONDITION'
,p_internal_uid=>12792603199100000
);
wwv_flow_imp_page.create_page_process(
...
'  IF v_id_caja IS NULL THEN',
'    apex_error.add_error(',
'      p_message => ''No tenes una caja abierta. Abri tu caja desde Apertura de Caja antes de facturar.'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'    RETURN;',
'  END IF;',
