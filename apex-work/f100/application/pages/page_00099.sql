...
'       case when d.ESTADO = ''PAGADA''',
'             and WKSP_WORKPLACE.FN_GET_PARAMETRO(''REVERSO_COBRO_ACTIVO'',''TEXTO'') = ''S'' then',
'         ''<a href="''||',
...
,p_value_alignment=>'LEFT'
,p_attributes=>wwv_flow_t_plugin_attributes(wwv_flow_t_varchar2(
  'html_expression', '<span class="t-Label t-Label--badge u-color-{case ''&ESTADO.'' when ''PENDIENTE'' then ''21'' when ''PAGADA'' then ''24'' else ''14'' end}">&ESTADO.</span>')).to_clob
,p_filter_is_required=>false
,p_use_as_row_header=>false
,p_enable_sort_group=>true
,p_enable_control_break=>true
...
wwv_flow_imp_page.create_region_column(
 p_id=>wwv_flow_imp.id(22999054543484107)
,p_name=>'FECHA_PAGO'
,p_source_type=>'DB_COLUMN'
...
wwv_flow_imp_page.create_region_column(
 p_id=>wwv_flow_imp.id(22999179914484108)
,p_name=>'NRO_RECIBO'
,p_source_type=>'DB_COLUMN'
...
wwv_flow_imp_page.create_ig_report_column(
 p_id=>wwv_flow_imp.id(23027331835366502)
,p_view_id=>wwv_flow_imp.id(16182673596278070)
,p_display_seq=>8
,p_column_id=>wwv_flow_imp.id(22999054543484107)
,p_is_visible=>true
,p_is_frozen=>false
);
wwv_flow_imp_page.create_ig_report_column(
 p_id=>wwv_flow_imp.id(23028266672366507)
,p_view_id=>wwv_flow_imp.id(16182673596278070)
,p_display_seq=>9
