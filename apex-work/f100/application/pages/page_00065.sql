...
wwv_flow_imp_page.create_page_item(
 p_id=>wwv_flow_imp.id(12747813944100000)
,p_name=>'P65_EMPLEADO_NOMBRE'
...
,p_item_plug_id=>wwv_flow_imp.id(12006213301524731)
,p_item_source_plug_id=>wwv_flow_imp.id(12747009310199339)
,p_item_default=>'SELECT V(''APP_USER'') FROM DUAL'
,p_item_default_type=>'SQL_QUERY'
...
wwv_flow_imp_page.create_page_process(
 p_id=>wwv_flow_imp.id(12760000000000000)
,p_process_sequence=>5
,p_process_point=>'BEFORE_HEADER'
,p_process_type=>'NATIVE_PLSQL'
,p_process_name=>'Validar caja del usuario'
,p_process_sql_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'DECLARE',
'  v_id_caja  NUMBER;',
'  v_id_emp   NUMBER;',
'BEGIN',
'  -- Verifica que el usuario APEX este vinculado a un empleado',
'  BEGIN',
'    SELECT ID_EMPLEADO INTO v_id_emp',
'      FROM WKSP_WORKPLACE.EMPLEADOS',
'     WHERE UPPER(CODIGO_USUARIO) = UPPER(V(''APP_USER''));',
'  EXCEPTION WHEN NO_DATA_FOUND THEN',
'    apex_error.add_error(',
'      p_message => ''Tu usuario APEX no esta vinculado a un empleado. Contacta al administrador.'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'    RETURN;',
'  END;',
'',
'  -- Regla unica: no abrir si ya hay una caja ABIERTA (de cualquier fecha)',
'  v_id_caja := WKSP_WORKPLACE.FN_CAJA_ABIERTA_USUARIO(V(''APP_USER''));',
'  IF v_id_caja IS NOT NULL THEN',
'    apex_error.add_error(',
'      p_message => ''Ya tenes una caja abierta (ID '' || v_id_caja || ''). Cerrala desde Cierre de Caja antes de abrir una nueva.'',',
'      p_display_location => apex_error.c_inline_in_notification);',
'  END IF;',
'END;'))
,p_process_clob_language=>'PLSQL'
,p_error_display_location=>'INLINE_IN_NOTIFICATION'
,p_internal_uid=>12760000000000000
);
